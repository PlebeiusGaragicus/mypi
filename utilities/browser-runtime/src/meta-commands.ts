...
import { SAFE_DIRECTORIES, escapeRegExp } from './path-security';
    ...
    case 'focus': {
      ...
      try {
        ...
        if (process.platform === 'darwin') {
          const appNames = ['Chromium', 'Google Chrome', 'Google Chrome for Testing', 'Arc', 'Brave Browser'];
          ...
          try {
            execSync('wmctrl -a "Chromium"', { stdio: 'pipe', timeout: 3000 });
            activated = true;
          } catch {
            try {
              execSync('xdotool search --onlyvisible --class chromium windowactivate', { stdio: 'pipe', timeout: 3000 });
              activated = true;
            } catch { /* fall through */ }
          }
        }
        ...
        if (!activated) {
          return 'Could not bring browser to foreground. On Linux install wmctrl or xdotool.';
        }
