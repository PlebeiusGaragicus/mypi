...
import { detectChallengeOnPage, appendChallengeBanner } from './challenge-detection';
...
export async function handleWriteCommand(
  ...
  switch (command) {
    case 'goto': {
      ...
      let result = `Navigated to ${normalizedUrl} (${status})`;
      const challenge = await detectChallengeOnPage(page, response);
      if (challenge.detected && challenge.kind) {
        result = appendChallengeBanner(result, challenge.kind, page.url());
      }
      return result;
    }
    ...
    case 'cookie-import-browser': {
      const { importCookies, listDomains, listSupportedBrowserNames } = await import('./cookie-import-browser');
      ...
      if (hasAll) {
        ...
        const allDomainNames = domains.map((d: { domain: string }) => d.domain);
        ...
        return `Imported ${result.count} cookies across ${Object.keys(result.domainCounts).length} domains from ${browser}`;
      }

      throw new Error(
        `Usage: cookie-import-browser [browser] --domain <domain> [--profile <name>]\n` +
        `       cookie-import-browser [browser] --all [--profile <name>]\n` +
        `Supported browsers: ${listSupportedBrowserNames().join(', ')}`,
      );
    }
    case 'cleanup': {
      // Parse flags
