...
- **Presets** (`agents/*.yml`) — 11 shipped profiles: tool-less chat personas (`chat`, `direct`, `human`, `plato`), capability workers (`scout`, `write`, `code`, `web`, `classifier`, `judge`), and the `workflow` orchestrator. User/project overlays merge on top. → [Presets](presets.md)
...
- **Skills** (`shared/skills/`) — path-promoted CLIs for Tavily, Exa, arXiv, congress.gov, BTC price, ntfy, todo; browser automation via `$B`; humanizer prose editing. → [Skills](skills.md)
